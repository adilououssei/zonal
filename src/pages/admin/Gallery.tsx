                ...
                filtered.map((img) => (
                    ...
                    >
                      <div
                        ...
                        className="aspect-video overflow-hidden relative w-full cursor-pointer"
                      >
                        <img
                        ...
                        {img.imageCount > 1 && (
                          <div className="absolute top-2 right-2 bg-black/60 text-white text-xs font-medium px-2 py-0.5 rounded-full">
                            {(cardIndex[img.id] ?? 0) + 1} / {img.images?.length ?? 1}
                          </div>
                        )}
                        {img.images && img.images.length > 1 && (
                          <>
                            <button
                              ...
                              className="absolute left-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80 flex items-center justify-center text-gray-700 hover:bg-white transition-colors cursor-pointer shadow-sm z-10"
                            >
                              <ChevronLeft size={16} />
                            </button>
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); goCard(img.id, 1, img.images) }}
                              className="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/80 flex items-center justify-center text-gray-700 hover:bg-white transition-colors cursor-pointer shadow-sm z-10"
                            >
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 pointer-events-none">
                        <button
                          ...
                          className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-700 hover:text-primary transition-colors cursor-pointer pointer-events-auto"
                        >
                          <Eye size={18} />
                        </button>
                        <Link
                          to={`/admin/gallery/${img.id}/edit`}
                          className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-700 hover:text-primary transition-colors pointer-events-auto"
                        >
                          <Pencil size={18} />
                        </Link>
                        <button
                          onClick={() => handleDelete(img.id)}
                          className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-gray-700 hover:text-red transition-colors cursor-pointer pointer-events-auto"
                        >
                          <Trash2 size={18} />
                        </button>
                      </div>
            ...
            {lightbox.images.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/80 flex items-center justify-center text-gray-800 hover:bg-white transition-colors cursor-pointer shadow-lg z-10"
                >
                  <ChevronLeft size={24} />
                </button>
                <button
                  onClick={() => setLightbox((prev) => prev ? { ...prev, index: (prev.index + 1) % prev.images.length } : null)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-white/80 flex items-center justify-center text-gray-800 hover:bg-white transition-colors cursor-pointer shadow-lg z-10"
                >
