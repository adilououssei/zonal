              ...
              >
                    ...
                    <div className="space-y-5">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <ImageUploadButton uploading={uploadingField} onUpload={handleLogoUpload} />
                        </div>
                      </div>
                      <div>
                ...
                {activeTab === 'contact' && (
                  <div>
                    ...
                    <div className="space-y-5">
                      <div>
                        <label className={`${labelClass} flex items-center gap-2`}><Mail size={14} /> Email principal</label>
                        <input type="email" value={form.contactEmail} onChange={(e) => set('contactEmail', e.target.value)} className={inputClass} />
                      </div>
                      <div>
                        <label className={`${labelClass} flex items-center gap-2`}><Phone size={14} /> Téléphone</label>
                        <input type="text" value={form.contactPhone} onChange={(e) => set('contactPhone', e.target.value)} className={inputClass} />
                      </div>
                      <div>
                        <label className={`${labelClass} flex items-center gap-2`}><Phone size={14} /> Téléphone secondaire</label>
                      ...
                      <div>
                        <label className={`${labelClass} flex items-center gap-2`}><MapPin size={14} /> Adresse</label>
                        <input type="text" value={form.address} onChange={(e) => set('address', e.target.value)} className={inputClass} />
                      </div>
                      <div>
                        <label className={`${labelClass} flex items-center gap-2`}><MessageCircle size={14} /> Numéro WhatsApp</label>
...
function ImageUploadButton({ uploading, onUpload }: { uploading: string | null; onUpload: (e: React.ChangeEvent<HTMLInputElement>) => void }) {
  ...
  return (
    <>
      <button type="button" onClick={() => ref.current?.click()} className="flex items-center gap-1.5 text-primary text-small font-medium hover:text-primary-dark transition-colors">
        ...
        {uploading === 'logo' ? 'Upload...' : 'Changer le logo'}
      </button>
